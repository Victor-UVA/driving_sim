...
def call_telemetry():
	...
	print('Vehicle angular acceleration (roll, pitch, yaw):\n')
...
def New_Position_Command(cmd):
	yy=0
	while yy<5:								# number of iterations to send at 60hz yy = 1 is equivolent 													to 0.016666667 seconds
		check_state()
		time.sleep(sleepy_time)
		new_hex_cmd_type = hex_cmd_type('new_position')
		final_command1 = command_checksum(new_hex_cmd_type,cmd)
...
while uuu<1000:
	...
	cmd[0] = 16383 + float(vehicle_acc_roll) * gain_roll
	cmd[1] = 16383 + float(vehicle_acc_pitch) * gain_pitch
