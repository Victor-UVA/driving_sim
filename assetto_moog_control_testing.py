...
def main():
    global p_roll, p_pitch, p_yaw, p_d_roll, p_d_pitch, p_d_yaw, p_t
    asm = accSharedMemory()
    moog = MOOG()
    ...
    while True:
        ...
        if sm is not None:
            ...
            # save accelerations
            # acceleration = np.array((sm.Physics.g_force.x,sm.Physics.g_force.y,sm.Physics.g_force.z))
            acceleration = np.array((sm.Physics.g_force.z,-sm.Physics.g_force.x,sm.Physics.g_force.y))*9.80665
            ...
            d_roll = (p_roll-roll)/dt
            d_pitch = (p_pitch-pitch)/dt
            d_yaw = (p_yaw-yaw)/dt

            dd_roll = (p_d_roll-d_roll)/dt
            dd_pitch = (p_d_pitch-d_pitch)/dt
            dd_yaw = (p_d_yaw-d_yaw)/dt

            gyroscope = np.array((dd_roll,dd_pitch,dd_yaw))
            ...
            roll = int(32767/58 * (roll + 29))
            pitch = int(32767/66 * (pitch + 33))


            # send frame
            try:
                moog.command_dof(roll= roll, pitch= pitch)
            except: 
                print('Command Error')
