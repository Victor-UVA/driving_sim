...
class DrivingSim():
    def __init__(self):
        ...
        self.final_pitch = 16383
        self.final_roll = 16383
        self.assetto_link = False 
        
        time.sleep(1)
        assetto_thread = Process(target=self.assetto_loop)
        assetto_thread.start()
        # moog_thread = threading.Thread(target=self.moog_loop)
        # Assetto Section
        assetto_button = tk.Button(self.app, text='Link Assetto', command=self.assetto_startup)
    ...
    def assetto_startup(self):
        self.assetto_link = True
    ...
    def assetto_loop(self):
        ...
        self.tachometer_serial = serial.Serial(
            port = 'COM4',
            baudrate = 115200,
            timeout=0,
            rtscts=True
        )
        while True:# listen for lock or something
            ...
            if sm is not None:
                ...
                self.tachometer_serial.write(packet.encode('utf-8'))
                if self.moog.is_engaged():
                    ...
                    try:
                        # TODO: try locks
                        self.moog.command_dof(roll=final_roll, pitch=final_pitch)
